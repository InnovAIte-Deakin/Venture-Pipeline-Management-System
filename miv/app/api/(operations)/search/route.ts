...
import { CapitalActivityType, DocumentType, FundType, GEDSICategory } from '@prisma/client'
...
function enumValueFromSearch<T extends Record<string, string>>(enumObject: T, searchTerm: string): T[keyof T] | undefined {
  const normalized = searchTerm.trim().replace(/[\s-]+/g, '_').toUpperCase()
  return Object.values(enumObject).find((value) => value === normalized) as T[keyof T] | undefined
}
...
async function searchDocuments(searchTerm: string, isMobile: boolean): Promise<SearchResult[]> {
  try {
    const typeMatch = enumValueFromSearch(DocumentType, searchTerm)
    const filters: any[] = [
      { name: { contains: searchTerm, mode: 'insensitive' } },
    ]
    if (typeMatch) filters.push({ type: typeMatch })
...
async function searchFunds(searchTerm: string, isMobile: boolean): Promise<SearchResult[]> {
  try {
    const fundTypeMatch = enumValueFromSearch(FundType, searchTerm)
    const filters: any[] = [
      { name: { contains: searchTerm, mode: 'insensitive' } },
      { vintage: { contains: searchTerm, mode: 'insensitive' } },
    ]
    if (fundTypeMatch) filters.push({ fundType: fundTypeMatch })
...
async function searchGEDSIMetrics(searchTerm: string, isMobile: boolean): Promise<SearchResult[]> {
  try {
    const categoryMatch = enumValueFromSearch(GEDSICategory, searchTerm)
    const filters: any[] = [
...
async function searchCapitalActivities(searchTerm: string, isMobile: boolean): Promise<SearchResult[]> {
  try {
    const typeMatch = enumValueFromSearch(CapitalActivityType, searchTerm)
    const filters: any[] = [
