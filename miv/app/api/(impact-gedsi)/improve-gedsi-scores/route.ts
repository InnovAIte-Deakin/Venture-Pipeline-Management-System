...
export async function POST() {
  try {
    ...
    const portfolioStages: VentureStage[] = [
      ...
      VentureStage.SERIES_C,
      VentureStage.SEED,
    ]
    const ventures: VentureWithGEDSIMetrics[] = await prisma.venture.findMany({
      where: {
        stage: {
          in: portfolioStages
        }
    ...
    for (const venture of ventures) {
      ...
      for (const metric of venture.gedsiMetrics) {
        ...
        if (metric.currentValue < metric.targetValue * 0.5) {
          ...
          newStatus = MetricStatus.IN_PROGRESS
          ...
          newStatus = MetricStatus.COMPLETED
          ...
          newStatus = MetricStatus.VERIFIED
        } else {
      ...
      if (ventureUpdates > 0) {
        ...
        await prisma.activity.create({
          data: {
            ...
            userId: user.id,
            type: ActivityType.METRIC_UPDATED,
