...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      successResponse(transformedDocument, 'Document created successfully'),
      { status: 201 }
    )
  } catch (error) {
...
async function getDocumentsByStatus(where: any) {
  ...
  ventures.forEach((v) => {
    ...
    else if (v.stage === 'DUE_DILIGENCE' || v.stage === 'INVESTMENT_READY') {
      statusCounts.review += v._count._all
