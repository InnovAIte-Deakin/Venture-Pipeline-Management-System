// app/dashboard/(g1-impact-analytics)/performance-analytics/lib/calculations.ts
//
// T19 - Refactor and Improve Performance Analytics
// Formulas unchanged from the original file - pure structural relocation.
...
export function generateAIInsights(data: AnalyticsData) {
  ...
  if (gedsiCompliance >= 75) {
    insights.push({
      ...
      icon: CheckCircle, iconColor: "text-green-600", textColor: "text-green-900",
      descriptionColor: "text-green-700", borderColor: "border-green-500"
    })
    ...
    insights.push({
      ...
      icon: AlertCircle, iconColor: "text-yellow-600", textColor: "text-yellow-900",
      descriptionColor: "text-yellow-700", borderColor: "border-yellow-500"
    })
  }
...
export function generateRiskAssessment(data: AnalyticsData) {
  ...
  return [{
    ...
    items: [
      { label: "Pipeline Risk", value: pipelineRisk, badgeClass: pipelineRisk === "Low" ? "bg-green-50 text-green-700" : pipelineRisk === "Medium" ? "bg-yellow-50 text-yellow-700" : "bg-red-50 text-red-700" },
      { label: "Compliance Risk", value: complianceRisk, badgeClass: complianceRisk === "Low" ? "bg-green-50 text-green-700" : complianceRisk === "Medium" ? "bg-yellow-50 text-yellow-700" : "bg-red-50 text-red-700" },
      { label: "Market Risk", value: marketRisk, badgeClass: marketRisk === "Low" ? "bg-green-50 text-green-700" : "bg-yellow-50 text-yellow-700" }
    ]
  }]
}
...
export function generateOptimizationOpportunities(data: AnalyticsData): string[] {
  ...
  const sectorStats = data.ventures.reduce((acc: any, venture) => {
    ...
    acc[sector].total++
    if (['FUNDED', 'SERIES_A', 'SERIES_B', 'SERIES_C'].includes(venture.stage)) acc[sector].funded++
    return acc
  }, {})

  const bestSector = Object.entries(sectorStats)
    .map(([sector, stats]: [string, any]) => ({ sector, successRate: stats.total > 0 ? (stats.funded / stats.total) * 100 : 0, total: stats.total }))
    .filter(s => s.total >= 2)
  ...
  const topLocation = Object.entries(locations).sort(([, a], [, b]) => (b as number) - (a as number))[0]
