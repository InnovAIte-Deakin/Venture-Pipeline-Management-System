...
- Source file: `miv/app/dashboard/(pipeline-management)/due-diligence/page.tsx`
- Public route: `/dashboard/due-diligence`
- Route group: `(pipeline-management)` is a Next.js route group and is not part of the URL.
...
- `miv/app/api/(pipeline-management)/ventures/route.ts`
...
- `npm.cmd exec eslint -- "app/dashboard/(pipeline-management)/due-diligence"`: passed with no errors or warnings.
...
- `npm.cmd run build`: failed due to unrelated existing issues:
  - Missing module `@/hooks/use-performance-analytics-data` imported by `app/dashboard/(analytics-insights)/performance-analytics/page.tsx`.
