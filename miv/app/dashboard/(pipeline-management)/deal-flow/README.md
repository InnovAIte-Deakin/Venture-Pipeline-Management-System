...
- Source file: `miv/app/dashboard/(pipeline-management)/deal-flow/page.tsx`
...
- `npm.cmd run build` with approved network access: compiled successfully, then failed during TypeScript on `app/api/(impact-gedsi)/improve-gedsi-scores/route.ts:15`, where `string[]` is not assignable to the Prisma `VentureStage[]` filter type.
