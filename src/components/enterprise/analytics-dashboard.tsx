...
function generateRealAlerts(ventures: any[]) {
  ...
  const riskVentures = ventures.filter(
    (v) => (v.gedsiScore && v.gedsiScore < 60) || v.status === 'INACTIVE' || v.stage === 'ON_HOLD',
  )
  ...
  const needReview = ventures.filter(
    (v) => v.stage === 'DUE_DILIGENCE' || v.stage === 'INVESTMENT_READY' || !v.lastReviewDate,
  )
  ...
  const highPotential = ventures.filter(
    (v) => v.gedsiScore >= 80 || v.stage === 'SERIES_A' || v.stage === 'SERIES_B',
  )
...
function generateRecentActivities(ventures: any[]) {
  ...
  const recentAssessments = ventures
    .filter((v) => v.gedsiScore && v.updatedAt)
  ...
  const recentFunding = ventures
    .filter((v) => v.fundingRaised > 0 && ['SERIES_A', 'SERIES_B', 'FUNDED'].includes(v.stage))
    .slice(0, 1)
...
export function AnalyticsDashboard({
  ...
  const [widgetConfig, setWidgetConfig] = useState<{
    ...
    try {
      ...
      return raw
        ? JSON.parse(raw)
        : { keyMetrics: true, performanceCharts: true, alertsPanel: true }
    } catch {
      return { keyMetrics: true, performanceCharts: true, alertsPanel: true }
    }
  })
  ...
  const persistWidgets = (cfg: typeof widgetConfig) => {
    setWidgetConfig(cfg)
    try {
      localStorage.setItem('analytics.widgets', JSON.stringify(cfg))
    } catch {}
  }
  const persistLayout = (mode: typeof layoutMode) => {
    setLayoutMode(mode)
    try {
      localStorage.setItem('analytics.layout', mode)
    } catch {}
  }
  ...
  const persistOptions = (updater: (curr: Record<string, any>) => Record<string, any>) => {
    setChartOptionsMap((prev) => {
      ...
      if (typeof window !== 'undefined')
        localStorage.setItem('analytics.chartOptions', JSON.stringify(next))
      return next
    })
  }
  ...
  const handleExport = () => {
    ...
    download(
      `dashboard-charts-${new Date().toISOString().slice(0, 10)}.json`,
  ...
  const ChartRenderer = ({ chart }: { chart: ChartWidget }) => {
    ...
    const palette = (mergedOptions.palette as string[]) || [
      '#6366F1',
      '#22C55E',
      '#F59E0B',
      '#EC4899',
    ...
    const barMaxWidth =
      typeof mergedOptions.barMaxWidth === 'number' ? mergedOptions.barMaxWidth : 36

    // Stacked or single-series rendering - moved outside switch to avoid conditional hook
    const stackedKeys: string[] = Array.isArray(mergedOptions.stackedKeys)
      ? mergedOptions.stackedKeys
      : []
    ...
    const transformedData = useMemo(() => {
      ...
      return (chart.data as any[]).map((row) => {
        ...
        stackedKeys.forEach((key) => {
          next[key] = Math.round(((Number(row[key]) || 0) / total) * 100)
        })
        return next
      })
      ...
      case 'bar':
        if (mergedOptions.layout === 'vertical') {
          const values = Array.isArray(chart.data)
            ? (chart.data as any[]).map((d) => Number(d[yKey]) || 0)
            : [0]
            ...
            <div style={{ width: '100%', height: heightPx }}>
              <ResponsiveContainer>
                ...
                >
                  ...
                  <XAxis
                    type="number"
                    domain={[0, stackedAsPercent && stackedKeys.length ? 100 : 'auto']}
                  />
                  ...
                  <Tooltip
                    formatter={(value: any, name: any, props: any) => {
                      ...
                      if (showDeltaLabels && typeof delta === 'number')
                        parts.push(`${delta >= 0 ? '+' : ''}${delta} vs prev`)
                      return [parts.join(' • '), '']
                    }}
                  />
                  {stackedKeys.length ? (
                    ...
                    <Bar
                      ...
                      maxBarSize={barMaxWidth}
                      onClick={(data: any) =>
                        mergedOptions.onBarClick && mergedOptions.onBarClick(data?.payload)
                      }
                      cursor={mergedOptions.onBarClick ? 'pointer' : 'default'}
                    >
                      {options?.labels !== false && (
                        <LabelList
                          ...
                          formatter={(val: any, _name: any, props: any) => {
                            ...
                            if (showDeltaLabels && typeof delta === 'number')
                              base = `${base}  ${delta >= 0 ? '▲' : '▼'}${Math.abs(delta)}`
                            return base
                          }}
                        />
                      )}
  ...
  return (
    <div className="space-y-6">
      ...
      {widgetConfig.keyMetrics && (
        <div
          className={`grid ${layoutMode === 'compact' ? 'grid-cols-2' : 'grid-cols-1 md:grid-cols-2'} ${layoutMode === 'wide' ? 'lg:grid-cols-5' : 'lg:grid-cols-4'} gap-6`}
        >
          {metrics.map((metric, index) => (
            <Card
              key={index}
              className="border-0 shadow-sm hover:shadow-lg transition-all duration-300 hover:scale-105 bg-gradient-to-br from-white to-gray-50/50"
            >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-baseline space-x-2">
                      ...
                      {metric.change !== 0 && (
                        <div
                          className={`flex items-center space-x-1 px-2 py-1 rounded-full ${getChangeColor(metric.changeType)} ${metric.changeType === 'increase' ? 'bg-emerald-50' : 'bg-red-50'}`}
                        >
                  ...
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center ${metric.color} transform transition-all duration-300 hover:scale-110 hover:shadow-xl`}
                  >
                    {metric.icon}
                  </div>
                </div>
      ...
      {widgetConfig.performanceCharts && (
        <div
          className={`grid grid-cols-1 ${layoutMode === 'wide' ? 'lg:grid-cols-3' : 'lg:grid-cols-2'} xl:grid-cols-3 gap-6`}
        >
          {charts.map((chart) => (
            <Card
              key={chart.id}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setIsCustomizing((curr) => !curr)}
                    >
              ...
              <CardContent>
                <div className="bg-white rounded-lg">
                  <ChartRenderer
                    chart={{
                      ...chart,
                      options: { ...(chart as any).options, ...(chartOptionsMap[chart.id] || {}) },
                    }}
                  />
                  ...
                  {isCustomizing && (
                    <div className="mt-4 flex flex-wrap items-center gap-3 text-sm">
                      ...
                      <Button
                        variant={customizingChartId === chart.id ? 'default' : 'outline'}
                        size="sm"
                        onClick={() =>
                          setCustomizingChartId(customizingChartId === chart.id ? null : chart.id)
                        }
                      >
                      ...
                      {customizingChartId === chart.id && (
                        <>
                          ...
                          {['#0EA5E9', '#22C55E', '#F59E0B', '#EF4444', '#8B5CF6', '#06B6D4'].map(
                            (c) => (
                              <button
                                key={c}
                                type="button"
                                onClick={() =>
                                  persistOptions((prev) => ({
                                    ...prev,
                                    [chart.id]: { ...(prev[chart.id] || {}), color: c },
                                  }))
                                }
                          ...
                          <label className="ml-4 flex items-center gap-2">
                            <input
                              type="checkbox"
                              checked={chartOptionsMap[chart.id]?.showConversionLabels ?? true}
                              onChange={(e) =>
                                persistOptions((prev) => ({
                                  ...prev,
                                  [chart.id]: {
                                    ...(prev[chart.id] || {}),
                                    showConversionLabels: e.target.checked,
                                  },
                                }))
                              }
                            />
                          ...
                          <label className="flex items-center gap-2">
                            <input
                              type="checkbox"
                              checked={chartOptionsMap[chart.id]?.showDeltaLabels ?? true}
                              onChange={(e) =>
                                persistOptions((prev) => ({
                                  ...prev,
                                  [chart.id]: {
                                    ...(prev[chart.id] || {}),
                                    showDeltaLabels: e.target.checked,
                                  },
                                }))
                              }
                            />
                          ...
                          <label className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              value={chartOptionsMap[chart.id]?.yAxisWidth ?? 160}
                              onChange={(e) =>
                                persistOptions((prev) => ({
                                  ...prev,
                                  [chart.id]: {
                                    ...(prev[chart.id] || {}),
                                    yAxisWidth: Number(e.target.value),
                                  },
                                }))
                              }
                            />
                          ...
                          <label className="flex items-center gap-2">
                            ...
                            <input
                              type="number"
                              className="w-20 border rounded px-2 py-1"
                              value={chartOptionsMap[chart.id]?.barMaxWidth ?? 42}
                              onChange={(e) =>
                                persistOptions((prev) => ({
                                  ...prev,
                                  [chart.id]: {
                                    ...(prev[chart.id] || {}),
                                    barMaxWidth: Number(e.target.value),
                                  },
                                }))
                              }
                            />
                          ...
                          <label className="flex items-center gap-2">
                            ...
                            <input
                              type="checkbox"
                              checked={chartOptionsMap[chart.id]?.stackedAsPercent ?? false}
                              onChange={(e) =>
                                persistOptions((prev) => ({
                                  ...prev,
                                  [chart.id]: {
                                    ...(prev[chart.id] || {}),
                                    stackedAsPercent: e.target.checked,
                                  },
                                }))
                              }
                            />
                ...
                <div className="space-y-2">
                  <label className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      className="rounded"
                      checked={widgetConfig.keyMetrics}
                      onChange={(e) =>
                        persistWidgets({ ...widgetConfig, keyMetrics: e.target.checked })
                      }
                    />
                  ...
                  <label className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      className="rounded"
                      checked={widgetConfig.performanceCharts}
                      onChange={(e) =>
                        persistWidgets({ ...widgetConfig, performanceCharts: e.target.checked })
                      }
                    />
                  ...
                  <label className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      className="rounded"
                      checked={widgetConfig.alertsPanel}
                      onChange={(e) =>
                        persistWidgets({ ...widgetConfig, alertsPanel: e.target.checked })
                      }
                    />
                ...
                <div className="space-y-2">
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={() => persistLayout('standard')}
                    />
                  ...
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={() => persistLayout('compact')}
                    />
              ...
              <div>
                ...
                <div className="space-y-2">
                  <Button
                    ...
                    onClick={() => {
                      try {
                        localStorage.setItem('analytics.widgets', JSON.stringify(widgetConfig))
                        localStorage.setItem('analytics.layout', layoutMode)
                      } catch {}
